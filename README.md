...
3. **ESP32 Main Brain** (`ESP32_MAIN_BRAIN/ESP32_MAIN_BRAIN.ino`) receives the classification over HTTP. Actuator/servo control to physically route the item is stubbed with a `TODO` — not yet implemented.
...
2. **Flash the firmware** via Arduino IDE — `ESP32_CAM.ino` to the ESP32-CAM module (needs the `esp32` board package for the camera driver), `ESP32_MAIN_BRAIN.ino` to the second ESP32 (needs the `ArduinoJson` library).
...
- Actuator control on the Main Brain (moving the bin mechanism) is not implemented — classification and signaling work end-to-end, sorting hardware does not.
